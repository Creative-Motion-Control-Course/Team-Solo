...
Button button_d1;       // D1 - mode switch button
...
void draw_step() {
  ...
  if (mode == 0) {
    // Mode 0: Wave - LFO controls sine wave amplitude
    x = t * 2.0;
    y = sin(t) * signal * 10.0;
    ...
    // Mode 1: Noise - wave with random noise
    x = t * 2.0;
    ...
    // Mode 2: Drift - signal accumulates, line drifts
    x = t * 2.0;
    ...
    // Mode 3: Flower - TBI queues petal paths, LFO controls size
    if (!flower_queued) {
      ...
      for (int i = 0; i < petals; i++) {
        float angle = (2.0 * PI * i) / petals;
        float px = cx + cos(angle) * radius;
        float py = cy + sin(angle) * radius;

        time_based_interpolator.add_move(GLOBAL, 15.0, cx, cy, 0, 0, 0, 0);
        time_based_interpolator.add_move(GLOBAL, 15.0, px, py, 0, 0, 0, 0);
        time_based_interpolator.add_move(GLOBAL, 15.0, cx, cy, 0, 0, 0, 0);
      }
      flower_queued = true;
    }
